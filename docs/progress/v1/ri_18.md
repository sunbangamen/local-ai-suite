...
- **Tests**: `test_approval_workflow.py` (9개 시나리오)
...
| Task | Description | DoD | Risk |
|------|-------------|-----|------|
| pytest 실행 | `cd services/mcp-server && pytest tests/test_approval_workflow.py -v` | 9/9 통과 | Low |
| 로그 저장 | pytest 결과를 `docs/progress/v1/APPROVAL_WORKFLOW_TEST_RESULTS.md`에 저장 | 파일 생성 확인 | Low |
| 문서 업데이트 | IMPLEMENTATION_SUMMARY.md에 테스트 명령 추가 | 테스트 실행 명령 명시 | Low |
| 스냅샷 생성 | 테스트 통과율, 실행 시간 요약 | "9/9 passed (100%)" 기록 | Low |
...
- **Integration Tests**: 현재 9개 시나리오 통합 테스트
...
**3. 통합 테스트 (pytest)**
```bash
# TC7: Metadata validation test (seconds_until_expiry field verification)
pytest tests/test_approval_workflow.py::test_wait_for_approval_provides_request_metadata -v
# Validates that approval request metadata includes seconds_until_expiry field
# Asserts that seconds_until_expiry value is > 0 (not always 0)
# Verifies seconds_until_expiry in both DB view and API response context

# TC8: Performance validation test
pytest tests/test_approval_workflow.py::test_performance_bulk_approvals -v
# Verifies latency targets for bulk approval operations

# TC9: Error handling test
pytest tests/test_approval_workflow.py::test_wait_for_approval_handles_creation_failure -v
# Verifies proper error handling and database state consistency
```

**Note on seconds_until_expiry Validation:**
The `seconds_until_expiry` field is explicitly validated in `test_wait_for_approval_provides_request_metadata` (scenario 7)
through assertions that confirm:
1. The field exists in the database view results (pending[0]["seconds_until_expiry"])
2. The value is positive (> 0), not always 0
3. The field is propagated to the API response context

This validation fulfills Issue #36 requirements for field unification and correctness.

### Performance Criteria
- **CLI 응답시간**: < 1초 (list 명령)
- **API 응답시간**: < 100ms (pending 엔드포인트)
- **테스트 실행시간**: < 10초 (9개 통합 테스트)
...
### Quality Review
- [x] **테스트 전략이 충분한가요?**
  - 9개 기존 통합 테스트 (승인, 거부, 타임아웃, 동시성, 권한, 감사, 성능, 정확도, CLI)
  - 문서 예제 재현 테스트 (E2E)
  - API 수동 검증 포함
