...
**3. 통합 테스트 (pytest)**
```bash
# TC8-9: Existing integration tests in test_approval_workflow.py
pytest tests/test_approval_workflow.py::test_approval_request_expiry_accuracy -v
pytest tests/test_approval_workflow.py::test_cli_single_run_display -v

# Expected behavior:
# - test_approval_request_expiry_accuracy: Validates database view calculation of seconds_until_expiry
# - test_cli_single_run_display: Verifies CLI list output format and Rich table rendering
```

**Note on seconds_until_expiry Validation:**
The `test_approval_request_expiry_accuracy` test (scenario 8) validates that the database view correctly
calculates `seconds_until_expiry`. Additional CLI-specific testing of the seconds_until_expiry field
is done in `test_cli_single_run_display` (scenario 9) via the display_requests() function.
