...
**목표**: Phase 3 전체 스택 정상 기동 및 9개 서비스 헬스체크 통과
...
| Task | Description | DoD | Risk |
|------|-------------|-----|------|
| `.env` 보안 설정 검토 | RBAC, Approval, Sandbox 설정 확인 | 프로덕션 권장 설정 문서화 | Low |
...
| API 응답 시간 확인 | Grafana p95 latency | < 3초 | Medium |
| GPU 메모리 사용률 확인 | `nvidia-smi` | < 90% | High |
...
- **Purpose**: Service health verification (Phase 2 - 9개 서비스)
