...
- CLI 승인 대기 핸들링 (`scripts/ai.py:178-324`):
  ...
  - 1초 폴링 간격으로 승인 상태 체크
  - 승인 시 자동 도구 재실행
- ⚠️ 현재 CLI는 `GET /api/approvals/{request_id}/status`를 메인 MCP 서버에 요청하지만,
  해당 엔드포인트는 보안 관리자 앱(`security_admin.py:537`)에만 존재하여 404가 발생함.
  메인 앱에 상태 조회 API를 노출하거나 CLI가 보안 관리자 호스트로 폴링하도록 경로를 조정해야 함.
...
- CLI 폴링 경로 수정 또는 메인 MCP에 `/api/approvals/{id}/status` 노출이 선행되어야 함

**예상 시간:** 1.5일 (상태 API 정비 포함)
**위험도:** Low
...
**선택 이유:**
1. **빠른 프로덕션 투입**: 문서와 경로 보완 작업만으로 즉시 운영 가능
2. **낮은 회귀 위험**: 추가 변경은 승인 상태 API 노출 및 환경 변수 체크 정도로 제한
3. **실용적 우선순위**: 배치 승인은 운영 경험 후 필요 시 추가
4. **Issue #26 완성도**: 핵심 기능은 이미 완료되어 추가 개발 범위가 작음
5. **현행 오류 해소**: CLI가 404를 받는 상태 API 경로 불일치를 동시에 해결
...
| Task | Description | DoD | Risk |
|------|-------------|-----|------|
| P2-T0 | 승인 상태 API 정비 | 메인 MCP에 `/api/approvals/{id}/status` 노출 또는 CLI가 보안 관리자 호스트로 폴링하도록 설정 | Medium |
| P2-T1 | ai.py 환경 변수 체크 | `APPROVAL_WORKFLOW_ENABLED` 체크 로직 추가 (20줄), false 시 승인 스킵 | Low |
| P2-T2 | --approvals 명령 추가 | 사용자가 대기 중인 자신의 승인 요청 조회 가능 | Low |
| P2-T3 | 에러 처리 개선 | 승인 타임아웃 시 명확한 안내 메시지, 재시도 가이드 제공 | Low |

**완료 기준:**
- [ ] `APPROVAL_WORKFLOW_ENABLED=false` 시 승인 단계 스킵
- [ ] `APPROVAL_WORKFLOW_ENABLED=true` 시 정상 동작
- [ ] 환경 변수 미설정 시 기본값(false) 사용
- [ ] CLI 폴링 시 404 미발생 (보안 관리자 앱과 경로 정합)
...
| 승인 상태 API 경로 불일치 | Medium | Medium | 메인 MCP에 `/status` 엔드포인트 추가 또는 CLI 폴링 URL을 보안 관리자 앱으로 조정 |
...
1. **Plan Approval** ✅
   - 위 체크리스트 모두 검토 완료
   - Option 1 (최소 운영 준비) 확정

2. **Issue Update**
   - GitHub Issue #40에 이 계획서 댓글 추가
   - Phase별 체크박스로 진행상황 추적

3. **Task Creation**
   - Phase 1-4 세부 작업을 TODO로 분할
   - 우선순위: P1 (문서) → P2 (CLI) → P3 (검증) → P4 (배포)

4. **Timeline Setup**
   - 시작일: 2025-10-24 (목)
   - 목표일: 2025-10-26 (금)
   - Phase 1 완료 후 운영팀 리뷰 요청
...
- ✅ **예상 일정**: 약 1.6일 (2025-10-24 → 2025-10-26)
- ✅ **위험도**: Low (코드 변경 최소, 기존 기능 99% 유지)

**주요 작업:**
1. **문서 4개** 작성/개선 (OPERATIONS_GUIDE 신규, IMPLEMENTATION_SUMMARY 확장)
2. **승인 상태 API/CLI 정합**: `/status` 엔드포인트 노출 및 환경 변수 체크 로직 추가
3. **E2E 테스트** 2개 시나리오 수동 검증
