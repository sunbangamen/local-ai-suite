...
| Task | Description | DoD | Risk |
|------|-------------|-----|------|
| P4-T1 | 테스트 환경 준비 | `make up-p3` 실행, DB 시딩 완료, 환경 변수 `APPROVAL_WORKFLOW_ENABLED=true` 반영 | Low |
| P4-T2 | 승인 요청 생성 테스트 | `ai --mcp execute_python` 실행, 403 응답 및 승인 요청 생성 확인 | Low |
| P4-T3 | 승인 처리 테스트 | `python scripts/approval_cli.py --continuous` 또는 기본 실행으로 대기 요청 조회 후 인터랙티브하게 승인 처리 | Low |
| P4-T4 | 자동 재실행 검증 | CLI에서 Short ID 선택 → `approve`/`reject` 입력 후 자동 폴링 및 재실행 확인 | Medium |
| P4-T5 | 로그 수집 및 문서화 | 스크린샷, DB 쿼리 결과, 타임스탬프 기록 | Low |

**신규 파일**:
- `docs/progress/v1/fb_21_validation.md` (검증 결과)
...
2. ⏳ **Phase 2 실행**: README, IMPLEMENTATION_SUMMARY, .env.example 업데이트
3. ⏳ **Phase 3 실행**: docs/runbooks/approval_workflow.md 작성
4. ⏳ **Phase 4 실행**: 실사용 검증 및 fb_21_validation.md 작성
5. ⏳ **PR 생성**: GitHub PR 생성 (자동화 없음, 수동 실행 필요 시)
...
| Phase 2 | 문서 업데이트 | 2시간 | - | ⏳ 대기 | - |
| Phase 3 | 운영 가이드 작성 | 4시간 | - | ⏳ 대기 | - |
| Phase 4 | 실사용 검증 | 3시간 | - | ⏳ 대기 | - |
...
**신규 문서**:
- [ ] docs/runbooks/approval_workflow.md (500-700줄 운영 가이드)
- [ ] docs/progress/v1/fb_21_validation.md (검증 결과)

**검증 산출물**:
- [ ] 터미널 스크린샷 (승인 UI, 재실행 메시지)
- [ ] DB 쿼리 결과 (approval_requests, security_audit_logs)
- [ ] 타임스탬프 기록 (요청/승인/재실행 시각)

---

## 💡 진행 방향

Issue #42의 모든 Acceptance Criteria를 충족하는 **완전한 운영화** 계획을 수립했습니다.

**작업 순서**:
1. Phase 2: 문서 일관성 확보 (README, IMPLEMENTATION_SUMMARY 업데이트)
2. Phase 3: 운영 가이드 작성 (approval_workflow.md 신규 생성)
3. Phase 4: 실사용 검증 및 결과 문서화 (테스트 실행, 로그 수집)

**예상 완료일**: 2025-10-25 (1-1.5일)
