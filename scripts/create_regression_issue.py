...
class GitHubIssueCreator:
    ...
    def parse_regression_report(self, report_path: str) -> Dict:
        ...
        warnings_match = re.search(r"### ⚠️ Warnings.*?(?=###|$)", content, re.DOTALL)
        warnings_lines = []
        if warnings_match:
...
"""
    ...
    def _format_table_lines(self, lines: List[str]) -> str:
        ...
        return "\n".join(line for line in lines if line.strip() and not re.match(r"^\|\s*-+", line))
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                if "already exists" in str(error_data):
                    print("⚠️ Issue already exists for this regression")
                    return None
                else:
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python scripts/create_regression_issue.py <regression_report_file> [--token TOKEN]"
        )
        print("\nExample:")
        print(
            "  python scripts/create_regression_issue.py load-test-results/regression-analysis.md"
